# Основные приципы тестирования в Rails 4 (конспект)
...
10. [**Интеграционное тестирование с Capybara и Cucumber**](#user-content-chapter-10)
  ...
  * [Как заставить пройти тест Capybara](#user-content-chapter-10.5)
...
RSpec.describe ProjectsController, type: controller do
  describe "POST create" do
    ...
    expect(responce).to redirect_to(projects_path)
...
В тесте контроллера можно получить доступ к объекту запроса через переменную *@request*, а к объекту контроллера через *@controller*. Так же есть объект *@responce*. Например, можно получить HTTP-headers через *@request.headers*.
...
У теста контроллера есть три вещи, которые можно проверять после действия контроллера:
  1. Возвращается ли ожидаемый НТТР-статус? RSpec предоставляет для этого *responce.status* объект и *have_http_status* сличитель (matcher);
...
it "is a successful index request with no filters" do
  ...
  expect(respomce).to render_template(:index)
end
```
...
```ruby
Failure/Error: expect(responce).to
...
### Как заставить пройти тест Capybara
